...
void TCesna::GetCmtyVV(TVec<TIntV>& CmtyVV, TVec<TFltV>& Wck, const double Thres, const int MinSz) {
  ...
  FILE* FX = fopen((OutFPrx + "affiliation_weights.txt").CStr(), "wt");
  for (int u = 0; u < F.Len(); u++) {
    for (int c = 0; c < NumComs; c++) {
      fprintf(FX, "%f", GetCom(u,c));
      if (c < NumComs - 1) { fprintf(FX, "\t"); }
    }
    fprintf(FX, "\n");
  }
  fclose(FX);

  FILE* FY = fopen((OutFPrx + "affiliation_weightsF.txt").CStr(), "wt");
  for (int u = 0; u < F.Len(); u++) {
    for (int c = 0; c < F[u].Len(); c++) {
      fprintf(FY, "%f", F[u][c].Val);
      if (c < F[u].Len() - 1) { fprintf(FY, "\t"); }
    }
    fprintf(FY, "\n");
  }
  fclose(FY);
...
#pragma omp parallel for
  ...
  printf("\nMLE completed with %d iterations(# secs)\n", iter);
